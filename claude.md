...
### Cosmetic tab
- Appearance, Backstory and Notes are three rich-text fields on the sheet's Cosmetic tab, stored at `system.details.appearance.value`, `system.details.biography.value` and `system.details.notes.value`. They're pure template additions — no `getData` fields, no listeners, no data model, because pbta's own `ActorDataTemplate` (`pbta/src/module/data/actor/templates/actor.js`) already defines `details` as an unrestricted `MappingField` of `{label, value}` entries with no `initialKeys` lock, the exact mechanism `system.details.callsign` already relies on. Any new key just works.
