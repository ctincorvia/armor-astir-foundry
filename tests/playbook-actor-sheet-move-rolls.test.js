...
describe("PlaybookActorSheet#_onMoveRoll - bite the dust's locked Desperation", () => {
	...
	it("locks Desperation when at max Dangers and every one is a Peril", async () => {
		...
		expect(configureMoveRoll).toHaveBeenCalledWith(BITE_THE_DUST, [defy], { lockedEffect: "desperation", lockedAdvantage: null, lockedTrait: null, astirPartSpends: [], equipmentSpends: [], rollModifiers: [], rollStack: null, disadvantageConversion: null });
	});
	...
	it("does not lock Desperation when at max Dangers but the types are mixed", async () => {
		...
		expect(configureMoveRoll).toHaveBeenCalledWith(BITE_THE_DUST, [defy], { lockedEffect: null, lockedAdvantage: null, lockedTrait: null, astirPartSpends: [], equipmentSpends: [], rollModifiers: [], rollStack: null, disadvantageConversion: null });
	});
...
describe("PlaybookActorSheet#_onMoveRoll - Bureaucrat's quick-roll redirect to Exchange Blows", () => {
	it("rolls the real Exchange Blows move with TALK forced and carries Bureaucrat's own reminders", async () => {
		...
		expect(configureMoveRoll).toHaveBeenCalledWith(
			...
			{ lockedEffect: null, lockedAdvantage: null, lockedTrait: talk, astirPartSpends: [], equipmentSpends: [], rollModifiers: [], rollStack: null, disadvantageConversion: null }
		);
